...
| **Support SLA** | Email, best-effort, ~1 business day initial-response target (`docs/DESK_LICENSE_TERMSHEET.md` §6.2) | Dedicated Slack Connect channel plus priority email; four-hour best-effort critical-incident target during CME/Eurex/LSE hours (`docs/ANIMUS_ENTERPRISE_TERMSHEET.md` §6.1) | Per OEM order form; typically matched to Institutional Enterprise for the OEM's own engineering team | Direct founder-level Sev-1 response, 2-hour initial response commitment during daily coverage hours (07:00–21:00 IST); 24/7/365 escalation available as a negotiated addendum |
...
| **Source access** | Not included — no exceptions at this tier (`docs/DESK_LICENSE_TERMSHEET.md` §3) | Not included by default; source-code **escrow** (continuity safeguard, not audit access) negotiable as a separate arrangement (`docs/ANIMUS_ENTERPRISE_TERMSHEET.md` §5.4) | Available under NDA as required for OEM integration and audit | Air-gapped RSA node licensing plus escrow / read-only source code inspection access for internal risk and security audits, under strict anti-fork and anti-compete IP covenants (`LEGAL_EULA.md` §3.1–3.2) |
...
- **Source access for internal security audit** is available under NDA at the OEM / Redistribution tier and above (Section 2) — Institutional Enterprise's own source provision is escrow only (a continuity safeguard releasable on vendor failure or uncured breach, not a standing audit right; Section 2's Source access row and `docs/ANIMUS_ENTERPRISE_TERMSHEET.md` §5.4). A customer needing to verify the engine's memory-safety and concurrency claims (`ARCHITECTURE.md` §2) directly, rather than take them on faith, should scope into OEM / Redistribution or Global Strategic Master Agreement, not assume audit access is included at Institutional Enterprise.
