> **This is the fintech/trading-desk track document** — the original project README, preserved here as-is with its full Phase 1-19 build/benchmark history. It moved out of `README.md` so the repository root could carry an aerospace/avionics-facing evaluation README instead; nothing below was rewritten. See `README.md` for the current top-level entry point.
...
With a well-chosen core, measured across 5 runs: **throughput and p50/p90 latency improve in every single trial** (15/15) -- real, consistent gains from cache locality and the simpler SPSC push path. p99 is a mixed bag (9/15 improved). **p99.99 originally got worse more often than not (12/15 trials)**, sometimes by 5-6x, and this did not go away with a properly probed, fast core. Root cause: `SetThreadAffinityMask`/`pthread_setaffinity_np` pin a thread but don't reserve a core *exclusively*, and the benchmark pinned without also raising the thread's scheduling priority -- an unpinned thread that hits contention can migrate to any idle core, but a pinned thread at default priority has nowhere to go until its one core frees up, and can still be preempted there by other normal-priority work.

**Fixed 2026-09-09:** `animus::sys::pin_current_thread_to_core_exclusive` (`include/animus/thread_affinity.hpp`, exported as `animus_pin_current_thread_to_core_exclusive`) pins *and* raises the thread to the host's highest realtime scheduling tier in one call, closing the exact gap above. Re-run across 5 fresh runs: **p99.99 improved in 15/15 trials** (full reversal), typically 8-75% lower, alongside the existing p50/p90/p99 gains. Still thread-affinity-plus-priority, not true OS-level exclusive core reservation (`isolcpus`/CPU Sets) -- not a formal real-time guarantee, but a measured, reproducible fix for the regression this phase originally found. See `AnimusCore_v1/BENCHMARKS.md`'s Phase 14 section for the full per-batch-size before/after numbers, the core-probe data, and the `alignas(64)` ctypes bug caught while building the SPSC drain path.
