...
- `PILOT_CONTRACT.md` §8.1's and `PILOT_AGREEMENT.md` §9.1's venue/arbitration forum are now both filled in with a matching clause — **SIAC arbitration, seated in Singapore, one arbitrator, English language**, with a carve-out for interim/injunctive relief in court pending tribunal constitution — but this is a founder-selected draft, not counsel-vetted, in both documents. Still open: whether SIAC is the right forum for the actual deal sizes each contract sees (vs. India-seated arbitration or straight litigation), and whether a Singapore-seated award is enforceable against the specific client jurisdictions actually in play.
- `PILOT_AGREEMENT.md` was intentionally left on its generic `[Licensor Legal Name]`/`[Jurisdiction]` placeholders otherwise (see §3.4) — only the arbitration clause was brought in line with the India-incorporation decision's dispute-resolution mechanism, not the entity name or governing-law jurisdiction.
...
>
> **Further resolution:** `PILOT_CONTRACT.md` §8.1 and `PILOT_AGREEMENT.md` §9.1 now both specify the same clause — **SIAC arbitration, seated in Singapore, one arbitrator, English language**, with an interim-relief carve-out — so an evaluator converting from the free Pilot into the paid Institutional Pilot Program sees the same dispute-resolution mechanism throughout. Both are founder-selected drafts, flagged for counsel confirmation on forum-fit and cross-border award enforceability rather than left blank.
...
**Remaining gap:** `LEGAL_EULA.md` §9.2 resolves venue only to "courts of competent jurisdiction in India" (litigation, country-level), while `PILOT_CONTRACT.md` and `PILOT_AGREEMENT.md` both now use SIAC arbitration — the free/paid evaluation templates and the production EULA use two different dispute-resolution mechanisms. Worth a decision on whether `LEGAL_EULA.md` should also move to arbitration for consistency, or whether litigation in India is intentionally the right mechanism for production license disputes specifically.
...
- [x] ~~Fill the venue/arbitration clause in `PILOT_CONTRACT.md` §8.1~~ — **RESOLVED (founder draft, not counsel-vetted):** SIAC arbitration, seated in Singapore, one arbitrator, English, with an interim-relief court carve-out. `PILOT_AGREEMENT.md` §9.1 now carries a matching clause. Confirm forum-fit and Singapore-award enforceability against actual client jurisdictions before relying on either.
- [ ] **Decide whether `LEGAL_EULA.md` §9.2 should also move to SIAC arbitration** for consistency with both pilot templates, or whether litigation in Indian courts is intentionally the right mechanism for production license disputes — new item, surfaced by the arbitration-clause rollout above.
