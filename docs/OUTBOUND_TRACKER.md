...
Geneva Trading and TransMarket Group are not among the 25 firms in [`ICP_TARGET_LIST.md`](ICP_TARGET_LIST.md) and have had no ICP scoring or hook-research pass — these contacts came in through direct LinkedIn networking rather than the batch process above. Tracked here so real outreach activity isn't lost, not folded into the Week 1–5 tables since the Tier/Technical Angle columns don't apply the same way.
...
- Geneva Trading (David Nuelle, Gaurav Mane) and TransMarket Group (Tom Norwood) — see "Ad Hoc Outreach" above — are the first outreach activity actually sent, ahead of Batch 1's planned 2026-09-08 first touch. All three are outside the 25-firm ICP list, so they're tracked separately rather than folded into Week 1.
