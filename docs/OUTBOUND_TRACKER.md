...
Geneva Trading, TransMarket Group, and Flow Traders are not among the 25 firms in [`ICP_TARGET_LIST.md`](ICP_TARGET_LIST.md) — these contacts came in through direct LinkedIn networking rather than the batch process above. Tracked here so real outreach activity isn't lost, not folded into the Week 1–5 tables since the Tier/Technical Angle columns don't apply the same way. Flow Traders specifically was researched and explicitly excluded from the 25 (`ICP_TARGET_LIST.md` §3: "some Python signal but not clearly in the live decision path") — worth keeping that caveat in mind before investing further here.
...
- Geneva Trading (David Nuelle, Gaurav Mane), TransMarket Group (Tom Norwood), and Flow Traders (Mark Atkinson, Stelios Evagorou) — see "Ad Hoc Outreach" above — are the first outreach activity actually sent, ahead of Batch 1's planned 2026-09-08 first touch. All are outside the 25-firm ICP list, so they're tracked separately rather than folded into Week 1; Flow Traders was previously researched and explicitly excluded (`ICP_TARGET_LIST.md` §3), so treat that connection as exploratory rather than a validated fit.
