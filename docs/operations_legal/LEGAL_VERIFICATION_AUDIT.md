...
>
> **Status update (2026-09-24 — supersedes the entity-*type* portion of the update below, entity-*existence* portion still stands):** the founder has since made a final call on entity **type**: Animus will file as a **One Person Company — "Animus Technologies (OPC) Private Limited"** (sole founder as the 100%-equity member/sole director, a family member as the statutory nominee under Form INC-3 with no operational role). This resolves the "second Director / nominee shareholder" blocker the Private-Limited path below could never close, by structural design rather than by finding a second person — see [`CORPORATE_INCORPORATION_CHECKLIST.md`](CORPORATE_INCORPORATION_CHECKLIST.md), now the **official** incorporation path, superseding [`CORPORATE_SETUP_PACKAGE.md`](CORPORATE_SETUP_PACKAGE.md)'s Private Limited package (kept in the repo as a historical draft / fallback if the OPC is later voluntarily converted to Private Limited, not deleted). **Not yet propagated:** every document below still names the entity **"Animus Technologies Private Limited"** without the `(OPC)` infix that MCA naming convention requires for this entity type — `../../LEGAL_EULA.md`, `../../COMMERCIAL.md`, `../../COMPLIANCE_AND_RISK_MITIGATION.md`, `PILOT_CONTRACT.md`, both term sheets, and `../../LEGAL_INCORPORATION_BRIEF.md` itself. This is a genuinely open follow-up, not resolved by this update — see the new item in §4's checklist. Do not rename these on the assumption it's a trivial find-and-replace: the actual registrable name still needs an MCA RUN availability check under the OPC naming rules before any document treats "Animus Technologies (OPC) Private Limited" as final.
>
...
> **Resolution (entity type corrected 2026-09-24 — see top-of-document banner):** the entity will be incorporated in India under the Companies Act, 2013, as **"Animus Technologies (OPC) Private Limited"** — a One Person Company, **not** the plain two-Member Private Limited Company this section originally described — see `CORPORATE_INCORPORATION_CHECKLIST.md`. `../../LEGAL_INCORPORATION_BRIEF.md` and the four downstream documents named just below still say "Private Limited Company" without the `(OPC)` infix and have **not yet been updated** to the corrected entity type — that rename is now the open item this paragraph flags, not a completed fact.
>
> **Superseded text (kept for the historical record, do not treat as current):** ~~the entity will be **Animus Technologies Private Limited**, a Private Limited Company under the Companies Act, 2013, incorporated in India — see `../../LEGAL_INCORPORATION_BRIEF.md`.~~ This is now reflected consistently in `../../LEGAL_EULA.md` (Vendor party block and signature block), `../../COMMERCIAL.md` §4.4 (Licensing entity), `../../COMPLIANCE_AND_RISK_MITIGATION.md` §3.2 (Contractor IP Assignment template), and `PILOT_CONTRACT.md` §Parties/signature block — all four now name the same (pre-OPC-correction) entity, with a `[CIN — to be inserted upon issuance of the Certificate of Incorporation]` placeholder pending actual filing. **`PILOT_AGREEMENT.md` was deliberately left on its generic `[Licensor Legal Name]` placeholder** and has not been updated to any entity name — see the entity-naming-consistency note in §3.4 below, now the one remaining gap on this point (in addition to the `(OPC)` rename, which affects the other four documents that *do* name the entity).
>
...
| `LEGAL_EULA.md` | Animus Technologies Private Limited, India (CIN pending) | **India** (§9.2 — already resolved) | **SIAC arbitration, seated in Singapore, 1 arbitrator, English** (§9.2, updated to match both pilot templates — founder draft, not counsel-vetted) |
...
- [ ] **Propagate the `(OPC)` entity-type correction** into `../../LEGAL_EULA.md`, `../../COMMERCIAL.md`, `../../COMPLIANCE_AND_RISK_MITIGATION.md`, `PILOT_CONTRACT.md`, both term sheets (`ANIMUS_ENTERPRISE_TERMSHEET.md`, `DESK_LICENSE_TERMSHEET.md`), and `../../LEGAL_INCORPORATION_BRIEF.md` — all currently name the entity "Animus Technologies Private Limited" (no `(OPC)` infix). Deliberately **not** done as part of this update: an MCA RUN name-availability check under OPC naming rules hasn't been run yet (`CORPORATE_INCORPORATION_CHECKLIST.md` §6 Step 1), so the exact registrable name isn't a settled fact to propagate. Do this rename pass once name reservation actually clears, not before.
