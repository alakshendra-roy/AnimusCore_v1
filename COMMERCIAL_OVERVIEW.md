...
| **Desk License** | $30,000 – $60,000 / yr | A single desk or engineering team deploying one production pipeline on one node | Single production node, single strategy or telemetry/surveillance pipeline, capped core count; compiled binaries; email support. Full terms: [`docs/operations_legal/DESK_LICENSE_TERMSHEET.md`](docs/operations_legal/DESK_LICENSE_TERMSHEET.md) |
| **Institutional Enterprise** | $240,000 / yr | A firm running Animus Core across multiple production nodes within one trading desk | Single desk, multi-node — unlimited cores/threads/packet volume within that desk; dedicated Slack Connect + priority email; source-code escrow negotiable (not standing source access). Full terms: [`docs/operations_legal/ANIMUS_ENTERPRISE_TERMSHEET.md`](docs/operations_legal/ANIMUS_ENTERPRISE_TERMSHEET.md) |
